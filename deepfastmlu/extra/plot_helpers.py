...
def plot_history_curves(history, show_min_max_plot, user_metric):
    ...
    _plot_curve(ax1, history, 'loss', 'Loss', show_min_max_plot)
    ...
    _plot_curve(ax2, history, train_metric_key, user_metric.capitalize(), show_min_max_plot)

    plt.tight_layout()
    plt.show()


def _plot_curve(ax, history, metric, ylabel, show_min_max_plot):
    """
    ...
    if show_min_max_plot:
        _plot_max_min_lines(ax, history, metric, ylabel)

    _set_plot_labels(ax, history, metric, ylabel)
...
def _plot_max_min_lines(ax, history, metric, ylabel):
    """
...
def _set_plot_labels(ax, history, metric, ylabel):
    """
